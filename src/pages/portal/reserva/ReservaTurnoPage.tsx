...
import { reglasDescuentoSesionApi } from '@/api/reglasDescuentoSesion'
import type {
  CodigoDescuentoDto,
  CuponDto,
...
import { useReservaAvailability } from './hooks/useReservaAvailability'
import { useReservaCatalog } from './hooks/useReservaCatalog'
import { useReservaPricing } from './hooks/useReservaPricing'
import { useReservaSteps } from './hooks/useReservaSteps'
import type { ReservaStepId, GuestData, SalonOption, SelectedSalonId, TimeGroupDefinition } from './types'
import { CALENDAR_WEEKDAYS, formatMonthLabel, startOfMonth } from './utils/reservaDates'
import { botanicalPattern, capitalize, formatCurrency, getDiscountMeta } from './utils/reservaFormatters'
import { DEFAULT_GUEST, isCompleteProfile } from './utils/reservaSelection'
...
export default function ReservaTurnoPage() {
  ...
  const profileComplete = isCompleteProfile(usuario)
  const showCouponLibrary = !!usuario
  const serviceSectionRef = useRef<HTMLDivElement | null>(null)
  const today = new Date()
  const [step, setStep] = useState<ReservaStepId>(profileComplete ? 'service' : 'guest-data')
  ...
  const [reglasDescuento, setReglasDescuento] = useState<ReglaDescuentoSesionPublicaDto[]>([])
  ...
  const {
    ...
    getPreviousStep,
    getVisibleIndex,
  } = useReservaSteps({ includeGuestData: !profileComplete })
  ...
  useEffect(() => {
    if (usuario) {
      ...
      setGuestData(DEFAULT_GUEST)
    }
  }, [usuario])

  useEffect(() => {
    if (profileComplete && step === 'guest-data') {
      setStep('service')
    }
  ...
  const {
    ...
    selectedService,
    selectionOptions,
  } = useReservaCatalog(selectedSex, selectedSalon, selectedServiceId)
  ...
  } = useReservaAvailability({
    stepId: step,
    selectedSalon,
    selectedServiceName: selectedService?.nombre ?? null,
    selectedCombo: selectionOptions.combos.find((combo) => combo.id === selectedComboId) ?? null,
    selectedZones: selectionOptions.zones.filter((zone) => selectedZoneIds.includes(zone.id)),
    calendarMonth,
  })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const data = await reglasDescuentoSesionApi.getPublicas()
        if (cancelled) return
        setReglasDescuento(data)
      } catch {
        if (cancelled) return
        setReglasDescuento([])
      }
  ...
  const reglaDescuentoServicio = selectedService
    ? reglasDescuento.find((regla) => regla.servicioId === selectedService.id) ?? null
    : null
  const {
    subtotal,
    estimatedDuration,
  ...
  const discountMeta = getDiscountMeta(selectedCoupon, appliedDiscountCode)
  const createdTotal =
  ...
  const progressIndex = getVisibleIndex(step)
  const progressValue = progressIndex >= 0 && visibleSteps.length > 1 ? (progressIndex / (visibleSteps.length - 1)) * 100 : 0
  const getStepKicker = (stepId: ReservaStepId) => {
    const index = getVisibleIndex(stepId)
    return index >= 0 ? `Paso ${index + 1}` : ''
  }
  const canProceed =
    step === 'guest-data'
      ? guestData.nombre.trim().length > 0 &&
        guestData.apellido.trim().length > 0 &&
  ...
  function resetSelectionsFromStep(targetStep: ReservaStepId) {
  ...
  const showSummary = step === 'selection' || step === 'schedule' || step === 'coupon'
  ...
  function renderStepContent() {
    if (step === 'guest-data') {
      return (
        <StepFrame
          kicker={getStepKicker('guest-data')}
    ...
    if (step === 'selection') {
      ...
      return (
        <StepFrame
          kicker={getStepKicker('selection')}
            ...
            <div className={`selection-column ${selectionMode === 'combos' ? 'mobile-hidden' : ''}`}>
              ...
              <div className={`discount-banner ${automaticDiscountPct > 0 ? 'active' : ''}`}>
                {automaticDiscountPct > 0
                  ? `✨ ¡Descuento del ${automaticDiscountPct}% aplicado!`
                  : reglaDescuentoServicio
                    ? `Seleccioná ${reglaDescuentoServicio.zonasMinimas} o más zonas y obtenés ${reglaDescuentoServicio.porcentajeDescuento}% automático`
                    : 'Seleccioná tus opciones disponibles'}
              </div>
        ...
        >
          ...
          <div className="manual-coupon-card">
            ...
            {appliedDiscountCode ? (
              ...
              <p className="manual-coupon-help">El beneficio se valida ahora, pero recién se consume cuando la reserva quede confirmada.</p>
            )}
    ...
    if (step === 'confirmation') {
      return (
        <StepFrame
          kicker={getStepKicker('confirmation')}
                ...
                {automaticDiscountValue > 0 || couponDiscountValue > 0 ? (
                  <div className="confirmation-discounts">
                    ...
                    {couponDiscountValue > 0 ? (
                      <div className="discount-line">
                        <div className="discount-copy">
                          <span>{discountMeta?.label ?? 'Promoción aplicada'}</span>
                          <small>{discountMeta?.description ?? 'El beneficio se consume cuando la reserva quede confirmada.'}</small>
                        </div>
  ...
  return (
    <>
      <main className={`reserva-page ${step === 'success' ? 'success-mode' : ''}`}>
        <div className="reserva-shell">
              ...
              <div className="progress-card">
                ...
                <div className="progress-steps">
                  {visibleSteps.map((visibleStep, index) => {
                    const currentVisibleIndex = getVisibleIndex(step)
                    const completed = currentVisibleIndex > index
                    const active = visibleStep.id === step
                    return (
                      <div key={visibleStep.id} className="progress-step">
          ...
          <div
            className={`content-shell ${!showSummary ? 'no-summary' : ''}`}
          >
            <div className={`content-column ${step === 'success' ? 'full-width' : ''}`}>
              {renderStepContent()}
              {step !== 'success' ? (
                <div className="wizard-footer">
                  <div className="footer-actions">
                    <Button variant="ghost" size="lg" onClick={goBack} leftIcon={<ArrowLeft size={16} />}>
                      {step === firstStepId ? 'Volver al inicio' : 'Anterior'}
                    </Button>
                    <Button
                      variant="primary"
                      size="lg"
                    ...
                    >
                      {step === 'confirmation' ? 'Confirmar turno' : 'Siguiente'}
                    </Button>
                  </div>
                </div>
              ) : null}
                ...
                <div className="summary-card">
                  ...
                  {automaticDiscountValue > 0 || couponDiscountValue > 0 ? (
                    <div className="summary-discounts">
                      ...
                      {couponDiscountValue > 0 ? (
                        <div className="summary-line discount">
                          <span>{discountMeta?.label ?? 'Promoción'}</span>
                          <strong>-{formatCurrency(couponDiscountValue)}</strong>
                        </div>
                      ) : null}
                    </div>
